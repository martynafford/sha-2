...
inline
void write_u32(uint8_t* dest, uint32_t x)
{
    *dest++ = x >> 24;
    *dest++ = x >> 16;
    *dest++ = x >> 8;
...
inline
void write_u64(uint8_t* dest, uint64_t x)
{
    *dest++ = x >> 56;
    *dest++ = x >> 48;
    *dest++ = x >> 40;
...
inline
sha256_hash sha256_impl(const uint32_t* s, const uint8_t* data, uint64_t length)
{
...
inline
sha512_hash sha512_impl(const uint64_t* s, const uint8_t* data, uint64_t length)
{
    static_assert(sizeof(uint32_t) == 4, "sizeof(uint32_t) must be 4");
    static_assert(sizeof(uint64_t) == 8, "sizeof(uint64_t) must be 8");
